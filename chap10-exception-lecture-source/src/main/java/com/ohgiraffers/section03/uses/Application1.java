...
public class Application1 {
    public static void main(String[] args) {
        ...
        * */
        System.out.println(new File("test.dat").getAbsolutePath());//외부파일이 프로젝트 안에 있으면 파일명만 적으면된다. (프로젝트 바로 밑(안)에 있다고 판단)
        //.getAbsolutePath() -> 파일의 절대경로를 추출
        BufferedReader br = null;

        try {
            br = new BufferedReader(new FileReader(new File("test1.dat")));//파일명이 잘못되면 br이 null이된다.
